...
const projectsData = [
   ...
   },
   {
      ...
      category: "Web Development",
      shortDesc: "A no-auth Django REST API powering an AI therapist mood journal — sub-second empathetic responses via GROQ's LLaMA 3.1, with persistent per-user history, multi-turn conversation context, and weekly wellness letters.",
      fullDesc: "MindEase is a mental wellness REST API built with Django REST Framework. Users submit mood entries (emoji + thoughts) and instantly receive warm, personalized AI responses from Luna — powered by GROQ's llama-3.1-8b-instant model on LPU infrastructure. No authentication required. The API supports multi-turn conversations by accepting optional conversation history for context-aware sessions, persists all journal entries per user with ISO 8601 timestamps (retrievable via history endpoint), generates weekly wellness summary letters with mood statistics, and ships with interactive Swagger and ReDoc documentation. Deployed live on Railway with auto-deploy from GitHub.",
      tags: ["Django", "Django REST Framework", "Python", "GROQ API", "LLaMA 3.1", "Railway", "OpenAPI", "Swagger"],
      ...
      features: [
         "AI-generated empathetic responses via GROQ LLaMA 3.1 (sub-second)",
         "Mood input: emoji + free-text thoughts",
         "Multi-turn conversation history — Luna maintains context across a session",
         "Persistent mood journal history per user with ISO 8601 timestamps",
         "Paginated history endpoint: GET /api/therapist/history/?user_id=",
         "Weekly wellness letter with mood statistics and personalized insights",
         "No authentication required — open REST API",
